...
cardekho_raw1_df = spark.read.format("csv").schema(cardekho_schema).option("header", "true") \
                                                                           .load("/mnt/cardekho-raw/cardekho_bronze_layer/cardekho/*")
...
cardekho_raw1_df.count()

# COMMAND ----------

from pyspark.sql.functions import col,when, regexp_replace, lower, trim,lit,regexp_extract,current_timestamp
...
location_null_df.repartition(1).write.mode("overwrite").format("delta").save("/mnt/cardekho-raw/bad-data/location_null_delta/")
...
wrong_price_df.repartition(1).write.mode("overwrite").format("delta").save("/mnt/cardekho-raw/bad-data/error_prices_of_car_delta/")
...
seating_missign_df.repartition(1).write.mode("overwrite").format("delta").save("/mnt/cardekho-raw/bad-data/seating_capacity_zero_delta/")
...
missed_values_df.repartition(1).write.mode("overwrite").format("delta").save("/mnt/cardekho-raw/bad-data/missed_values_delta/")
...
imgcount_zero_df.repartition(1).write.mode("overwrite").format("delta").save("/mnt/cardekho-raw/bad-data/imgcount_zero_delta/")
...
color_null_df.repartition(1).write.mode("overwrite").format("delta").save("/mnt/cardekho-raw/bad-data/color_null_delta/")
...
wrong_gearbox_df.repartition(1).write.mode("overwrite").format("delta").save("/mnt/cardekho-raw/bad-data/gearbox_null_delta/")
...
car_age_added_df.write.mode("overwrite").format("delta").save("/mnt/cardekho-raw/cardekho_silver_layer/car_age_added_delta/")